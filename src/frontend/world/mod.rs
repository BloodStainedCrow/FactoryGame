...
mod test {
    ...
    use crate::{blueprint::random_position, frontend::world::Position};
    ...
    proptest! {
        ...
        fn position_contained_in(position in random_position(), test_position in random_position(), size in (1u8..10, 1u8..10)) {
            ...
            prop_assume!(test_position.y < position.y + size.1 as usize);
        ...
        fn position_contained_in_sized(position in random_position(), test_position in random_position(), self_size in (1u8..100, 1u8..100), other_size in (1u8..100, 1u8..100)) {
            ...
            prop_assume!(test_position.y < position.y + other_size.1 as usize);
        ...
        fn super_stupid_test_unsized(position in random_position(), test_position in random_position(), size in (1u8..10, 1u8..10)) {
            ...
            for x_offs in 0..size.0 {
                for y_offs in 0..size.1 {
                    grid[position.x + x_offs as usize][position.y + y_offs as usize] = true;
                }
            }

            prop_assert_eq!(grid[test_position.x][test_position.y], test_position.contained_in(position, size))
        }
        ...
        fn super_stupid_test_unsized_sized(position in random_position(), test_position in random_position(), size in (1u8..3, 1u8..3), test_size in (1u8..3, 1u8..3)) {
            ...
            for x_offs in 0..test_size.0 {
                for y_offs in 0..test_size.1 {
                    hit |= grid[test_position.x + x_offs as usize][test_position.y + y_offs as usize];
                }
            }
