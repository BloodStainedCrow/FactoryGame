...
// use crate::saving::{save_at, load_at, save_at_fork};
...
type I = i32;
impl<T: 'static> SparseGrid<I, T> for BoundingBoxGrid<I, T> {
    fn new() -> Self {
        Self {
            extent: None,
            values: vec![],
    ...
    where
        T: Default,
    {
        // We do not use if let Some here due to borrow check issues
        if self.get(x, y).is_none() {
            let old = self.insert(x, y, T::default());
            assert!(old.is_none());
        }
        self.get(x, y).expect("Checked before")
    }
    ...
    fn insert(&mut self, x: I, y: I, value: T) -> Option<T> {
        ...
        let index = Self::calculate_index(
            self.extent.as_ref().expect("We just extended the extent"),
            [x, y],
        );
    ...
    fn occupied_entries(&self) -> impl Iterator<Item = ([I; 2], &T)> {
        self.values.iter().enumerate().filter_map(|(idx, v)| {
            let [x, y] = Self::calculate_pos(
                &self.extent.expect("If we have value we have an extent"),
                idx,
            );
    ...
    fn occupied_entries_mut(&mut self) -> impl Iterator<Item = ([I; 2], &mut T)> {
    ...
    fn insert_many(
        &mut self,
        positions: impl IntoIterator<Item = [I; 2]> + Clone,
        ...
        let x_min = positions
            ...
            .map(|[x, _y]| x)
            .min()
            .expect("We have at least one position");
        let x_max = positions
            .clone()
            .into_iter()
            .map(|[x, _y]| x)
        ...
        for (value, [x, y]) in values.into_iter().zip(positions) {
            let index = Self::calculate_index(
                self.extent
                    .as_ref()
                    .expect("If we have at least one position, we also have an extent"),
                [x, y],
            );
            debug_assert!(self.values[index].is_none());
        ...
        T: Send,
    {
        ...
        (0..(width * height))
            ...
            .map(|(x_offs, y_offs)| {
                (
                    top_left[0]
                        .checked_add_unsigned(x_offs.try_into().expect("Too large"))
                        .expect("Too large"),
                    top_left[1]
                        .checked_add_unsigned(y_offs.try_into().expect("Too large"))
                        .expect("Too large"),
                )
            })
