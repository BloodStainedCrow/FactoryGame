...
use super::{SparseGrid, bounding_box_grid::BoundingBoxGrid, map_grid::BtreeMapGrid};
...
pub struct DynamicGrid<I: Ord, T> {
...
type I = i32;
impl<T: 'static> SparseGrid<I, T> for DynamicGrid<I, T> {
    fn new() -> Self {
        Self {
            num_chunks: 0,
                ...
                if self.num_chunks * SWITCH_RATIO < new_size {
                    ...
                    take_mut::take(&mut self.store, |store| {
                        ...
                        let mut new_grid = BtreeMapGrid::new_with_values(
                            grid.get_extent().expect("To have the switch ratio be in play, we need chunks so we will have an extent"),
                            grid.into_iter(),
                        );
    ...
    fn insert_many(
        ...
        match &mut self.store {
            Backing::BoundingBoxGrid(grid) => {
                let [x_ext, y_ext] = grid.get_extent_after_insertion(positions.clone());
